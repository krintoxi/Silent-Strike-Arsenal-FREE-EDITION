...
$VARIABLES{'DIV'}        = "-" x 75;
$VARIABLES{'name'}       = "S.S.A Vulnerability Scanner";
$VARIABLES{'version'}    = "1...";
...
sub config_init {
    ...
    push(@CF,"$NIKTODIR/SSA.conf.default");
...
sub setup_dirs {
    ...
    chomp($CURRENTDIR);
    $CURRENTDIR =~ s#[\\/]vscan.pl$##;
    $CURRENTDIR = "." if $CURRENTDIR =~ /^vscan.pl$/;
    ...
    unless (defined $CONFIGFILE{'EXECDIR'}) {
        if (-d "$ENV{'PWD'}/plugins") {
            ...
            print STDERR "Could not work out the EXECDIR, try setting it in nikto.conf\n";
            exit;
        }
    }
