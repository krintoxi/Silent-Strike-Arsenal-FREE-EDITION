...
import sys,  re,  urllib.request,  urllib.parse,  urllib.error,  urllib.request,  urllib.error,  urllib.parse,  string
...
lines = ["NoobSecToolkit SQL Injection Vulnerability Scanner"]
print('------------------------------------------------------------------------------')
...
def HELP_PRNT():
    ...
    print("")
    print("Usage example:")
    print("sqli_scanner.py -u \"http://target.com/test.php?id=x\"")
    print("")
    print("Options:")
...
def URL_SCANNING(Site_URL):
    ...
    try:
        CALL_HTTP = urllib.request.urlopen(Get_URL)
    except HTTPError as  e:
    ...
    for index, item in enumerate(PARAMS):
        ...
        try:
            HTTP_CALL_TRIGGER_1 = urllib.request.urlopen(TRIG_URL_1)
        except HTTPError as  e:
            print("[!] The connection could not be established.")
            print("[!] Error code: ",  e.code)
        except URLError as  e:
        ...
        if len(SRCH_SQL_ERR_TRIGG_1) == 0 and len(SRCH_SQL_ERR_TRIGG_2) == 0 and len(SRCH_SQL_ERR_TRIGG_3) == 0:
            ...
            try:
                http_request_trigger_2 = urllib.request.urlopen(TRIG_URL_2)
            except HTTPError as  e:
                print("[!] The connection could not be established.")
                print("[!] Error code: ",  e.code)
            except URLError as  e:
